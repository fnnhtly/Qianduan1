...
const languageDict = {
    English: {
        ...
        importNotes: 'Import Notes (txt/json)'
    },
    zhongwen: {
        inputTitle: '请输入标题',
        inputContent: '请输入内容',
        ...
        if (element) {
            switch (button) {
                ...
                case '.flist5_daochu':
                    element.style.display = hasNotes && !showRecycleBin ? 'block' : 'none';
                    break;
                case '.flist3_huishouzhan':
                    element.style.display = hasNotes ? 'block' : 'none';
                    break;
            }
        }
    });
...
function renderNotes(query = '', showRecycleBin = false) {
    ...
    notes.forEach((note, index) => {
        ...
        if (showRecycleBin) {
            ...
            deleteButton.textContent = '删除至回收站';
...
function initApp() {
    ...
    recycleBinButton.addEventListener('click', function () {
        ...
        this.value = isRecycleBin ? '返回全部笔记' : '回收站';
    });
}
